...
`examples/` holds two fully-assessed fictional clients as importable engagement files — a PE-backed insurance broking roll-up and a PE-backed logistics group. They fail differently on purpose. See `examples/README.md`.

## Status / changelog

- **v1.7.1** — two worked examples added under `examples/`. Building them surfaced two gaps, both fixed: checklists had no way to record *"I checked and none of these exist"*, so confirmed absence read as unanswered; and a live exposure (an unauthorised agent emailing customers) was being sequenced into the 6–12 month phase by the dependency graph. Risk-flagged dimensions now emit a **containment action** in a *Before the roadmap* block, outside the sequence.
