...
Two fully-assessed fictional clients — a PE-backed insurance broking roll-up and a PE-backed logistics group — are **built into the tool**. Click **Worked examples** in the header and pick one; no file handling. They fail differently on purpose.

The same two are also in `examples/` as importable `.json` engagement files, for anyone who wants to edit or diff them. See `examples/README.md`.

## The client report

**Outputs tab → Client report — HTML / PDF.** Generates a single self-contained HTML document from the current engagement — cover, executive summary built around the ceiling, the exposure items, the scale explained, radar and scorecard, dimension-by-dimension with both risk framings, skills by cohort, the code audit where there is one, the sequenced plan with owners and definitions of done, and a method section that states what was *not* assessed.

Opens in a new tab with a **Print / save as PDF** button, or downloads as `.html`. No library, no server, works offline. Print settings: A4, default margins, tick *Background graphics*.

Two options before generating:

- **Name our sources against each finding** — off by default. Useful internally, risky in a client room, because a finding that contradicts the person who gave it becomes personal. The evidence base is summarised either way.
- **Include "How Enablis can help"** — on by default, as a clearly separated final section that opens by saying it is a proposal rather than a finding.

Samples: `examples/sample-report-ardent.html` and `examples/sample-report-ridgeway.html`.

## Status / changelog

- **v1.9** (package `1.15.0`) — **the client report rebuilt to the `design/mockup.html` visual system, one exhibit at a time.** The report now opens with an **executive-summary apex** (the finding in one line, plus foundation / exposure / candidate, all driven from the data), then a **stage-ladder hero** with every marker positioned from the scores, an **exposure board with a capacity meter** in *Fix these first*, a **maturity dot chart** in place of the 17-axis radar, each dimension **write-up as a finding exhibit** leading with the decision it forces, a **roadmap-on-a-page** (three horizons — contain now · a priority 90 days with Full AI Discovery inside it · beyond — with day estimates removed from the visual and the full per-action detail kept below), the **seam** as a green recommendation card naming the least-blocked ambition, and the method/research footer promoted to an **evidence-base credential** (four claim stats and the framework row, with the amplification-not-maturity caveat preserved). Finally, **every section heading is now a full-sentence, data-driven action title** in the mockup's voice — *"Readiness is Stage 1 of 5 — a control problem, not a capability one"*, *"Three things carry exposure today — they sit before the plan, not in it"* — falling back to the neutral topic heading where the client's own case can't carry a claim (a flat Stage-4 profile gets *"there is no single weak link to point at"*; a single-ambition client gets no seam claim at all). All eight worked examples regenerated; the report reads as one document and prints cleanly on A4 with no card breaking across a page. No change to what any dimension is scored or to the underlying model — this release is presentation.
- **v1.8.3** — **user journey stress-tested for two audiences it did not serve.** For someone who has never run discovery: a *New to this? Start here* primer above the swimlane — what you are actually doing, which of the three journeys to pick, the twelve terms the journey uses without defining (ceiling, target stage, cohorts, confirmed/told/guessed, spine and checklist, potential vs realistic, the probe, run capability, the gates, exposure vs maturity gap), and what to do before day one. For someone assessing a team inside a larger organisation: **every one of the 73 data rows now carries a fourth column, "if you can't get it"** — a proxy source or an honest way to record the gap. The journeys assumed an external pod with an executive sponsor who could summon Risk, Finance and the platform team; that assumption was load-bearing in every step and unavailable to most internal users. The primer also states the three things that change when you have no sponsor: assess what you can see and mark the rest N/A, treat an unanswerable question as a finding rather than a hole, and take the list of what you could not reach to leadership as the ask.
- **v1.8.2** — the client report now covers the **full toolkit**, not just readiness. Where an engagement has a workflow it adds *The workflow we went into* (the nine readiness signals, the ten-capability "could you run this without us?" scan, what is in the way, and where a person stays in control); where it has use cases it adds *The candidates, one by one* (each with value, readiness, hours, potential vs realistic value, cost, payback, what gates it and build-or-buy) and *What to do first, and what it is worth* (the ranking, the waves, and the aggregate with the potential-vs-realistic explanation). The method section now tells the truth in both cases — it previously claimed the assessment "does not price individual use cases", which was false the moment one was priced. Sample: `examples/sample-report-ardent-full.html`.
- **v1.8.1** — third worked example: **Ardent, full engagement** — the same client carried through Workflow and Use cases, so the full toolkit has a worked example too, not just the wedge. Building it exposed two calibration faults in the tool, both fixed. The use-case verdict thresholds sat a whisker above the minimum possible score (value ≥5 where the floor is 4, ease ≥15 where the floor is 12), so almost everything read "Do first" — now set to an average of 2 out of 3, and the four use cases land on three different verdicts. And use-case gating listed every organisational dimension below Stage 3, which on an early client is all of them; it now names the ceiling plus any mapped dimension the use case is also weak on, which took a ten-item list down to two.
- **v1.8** — **client-facing report generator.** Until now the outputs were plain text on the clipboard, so what a client saw depended on who pasted it. The Outputs tab now generates a complete branded HTML document that prints to PDF. Deliberate choices in it: source attribution is off by default; the method section states the scope *not* covered; unscored dimensions are named rather than quietly dropped; where the spine and checklist disagreed the report prints the disagreement instead of the flattering number; and the exposure items sit in their own section ahead of the roadmap. Two rendered samples added under `examples/`.
- **v1.7.1** — two worked examples, loadable from the header via a picker and also available as importable JSON under `examples/`. Replaces the single hardcoded Northwind example. Building them surfaced two gaps, both fixed: checklists had no way to record *"I checked and none of these exist"*, so confirmed absence read as unanswered; and a live exposure (an unauthorised agent emailing customers) was being sequenced into the 6–12 month phase by the dependency graph. Risk-flagged dimensions now emit a **containment action** in a *Before the roadmap* block, outside the sequence.
